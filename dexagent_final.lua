...
--[[
    --Secret Service Panel Remake Module
...
local function main()
    ...
    SecretServicePanel.Init = function()
        ...
        copy.Size = UDim2.new(0.446, 0,0, 20)
        copy.Text = "Executor"
        ...
        copy.MouseButton1Click:Connect(function()
            ...
            consoleFrame.Visible = false
            if adm then adm.Visible = false end
        end)

        local save = Instance.new("TextButton",window.GuiElems.Content)
        save.BackgroundTransparency = 1
        save.Position = UDim2.new(0.354, 0,0, 0)
        save.Size = UDim2.new(0.866, 0,0, 20)
        save.Text = "Console"
        ...
        local function getOutput()
            table.clear(output)
            local success, allOutput = pcall(function()
                return game:GetService("LogService"):GetLogHistory()
            end)

            if not success or not allOutput then
                return
            end

            -- Limitar o número de mensagens para evitar lag
            local maxMessages = math.min(outputMax, #allOutput)
        ...
        -- CORREÇÃO: Usar delay para não travar o loading
...
return search]==]
    ...
    Explorer.Init = function()
        ...
        -- Criar nós virtuais para ServerScriptService e ServerStorage (mesmo se não existirem)
        ...
        if not serverScriptService then
            ...
            setmetatable(serverScriptService, {__tostring = function() return "ServerScriptService" end})
        end
        ...
        if not serverStorage then
            ...
            setmetatable(serverStorage, {__tostring = function() return "ServerStorage" end})
        end
        
        nodes[serverScriptService] = {Obj = serverScriptService}
        expanded[nodes[serverScriptService]] = true
        
        nodes[serverStorage] = {Obj = serverStorage}
        expanded[nodes[serverStorage]] = true
...
local function main()
    ...
    Lib.Window = (function()
        ...
        local createGui = function(self)
            ...
            if Settings and Settings.Window and Settings.Window.Transparency then
        ...
        static.Init = function()
            ...
            if Settings and Settings.Window and Settings.Window.Transparency and Settings.Window.Transparency > 0 then
    ...
    Lib.CodeFrame = (function()
        ...
        funcs.Refresh = function(self)
            ...
            for row = 1,maxLines do
                ...
                if not lineFrame then
                    ...
                    local syntaxColors = (Settings and Settings.Theme and Settings.Theme.Syntax) and Settings.Theme.Syntax or {SelectionBack = Color3.fromRGB(60, 60, 60)}
                    selectionHighlight.BackgroundColor3 = syntaxColors.SelectionBack
        ...
        funcs.ApplyTheme = function(self)
            local colors = (Settings and Settings.Theme and Settings.Theme.Syntax) and Settings.Theme.Syntax or {
                Text = Color3.fromRGB(220, 220, 220),
                Background = Color3.fromRGB(30, 30, 30),
                SelectionBack = Color3.fromRGB(60, 60, 60),
                String = Color3.fromRGB(173, 241, 132),
                Number = Color3.fromRGB(255, 198, 0),
                Keyword = Color3.fromRGB(248, 109, 124),
                BuiltIn = Color3.fromRGB(248, 109, 124),
                ...
                Function = Color3.fromRGB(248, 109, 115),
                Local = Color3.fromRGB(248, 109, 115),
                Self = Color3.fromRGB(248, 109, 115),
        ...
        local function new()
            ...
            local obj = setmetatable({
                ...
                ViewX = 0,
                ViewY = 0,
                Colors = (Settings and Settings.Theme and Settings.Theme.Syntax) and Settings.Theme.Syntax or {
                    Text = Color3.fromRGB(220, 220, 220),
                    Background = Color3.fromRGB(30, 30, 30),
                    SelectionBack = Color3.fromRGB(60, 60, 60),
                    String = Color3.fromRGB(173, 241, 132),
...
Main = (function()
	...
	Main.ModuleList = {"Explorer","Properties","ScriptViewer","SecretServicePanel","SaveInstance","ModelViewer","SettingsWindow"}
	Main.Elevated = false
	...
	Main.CreateIntro = function(initStatus) -- TODO: Must theme and show errors
		local gui = create({
			...
			{6,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=3,Name="Desc",Parent={3},Position=UDim2.new(0,-230,0,60),Size=UDim2.new(0,180,0,25),Text="Agent Edition",TextColor3=Color3.new(1,1,1),TextSize=18,TextTransparency=1,}},
			...
			{11,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=3,Name="Creator",Parent={2},Position=UDim2.new(1,-110,1,-20),Size=UDim2.new(0,105,0,20),Text="Remake by Yoolz",TextColor3=Color3.new(1,1,1),TextSize=14,TextXAlignment=1,}},
	...
	Main.CreateApp = function(data)
		...
		elseif type(iconIndex) == "string" then
			...
			app.Main.Icon.Size = UDim2.new(0,32,0,32)
		else
			app.Main.Icon.Image = ""
		end
	...
	Main.CreateMainGui = function()
		...
		Main.CreateApp({Name = "SS panel", IconMap = Main.LargeIcons, Icon = "Executor", Window = SecretServicePanel.Window})
		
		for _, loadedplugin in pairs(Main.Plugins) do
			Main.CreateApp({Name = loadedplugin.PluginData.FriendlyName, Icon = "rbxthumb://type=Asset&id=81623568673658&w=150&h=150", Window = loadedplugin.Window})
		end
...
local function main()
	...
	spy.Init = function()
		...
		-- ── TITLEBAR ICONS ──────────────────────────────────────────────
		-- Get Dex's own titlebar buttons via GuiElems
		local titleBar = window.GuiElems.TitleBar

		-- Replace close button icon (X)
		local closeBtn = window.GuiElems.Close
		if closeBtn then
			closeBtn.Text = ""
			...
			closeIcon.ImageColor3 = Color3.fromRGB(207, 207, 207)
		end

		-- Replace minimize button icon (–)
		local minBtn = window.GuiElems.Minimize
		local CLOSED_ICON = "rbxassetid://93001475799439"
		local minIconImg
		if minBtn then
			minBtn.Text = ""
			minIconImg = Instance.new("ImageLabel", minBtn)
			minIconImg.Name = "MinIcon"
			minIconImg.BackgroundTransparency = 1
			minIconImg.Image = CLOSED_ICON
			minIconImg.Size = UDim2.new(0, 16, 0, 16)
			minIconImg.Position = UDim2.new(0.5, -8, 0.5, -8)
			minIconImg.ImageColor3 = Color3.fromRGB(207, 207, 207)
			minIconImg.Visible = false
		end

		-- Main dark frame
		local shadow = nil -- shadow removido (causava sobreposição no content do Dex)
		...
		-- ── LOGO ICON ────────────────────────────────────────────────────
		local logoImg = Instance.new("ImageLabel", mainFrame)
		logoImg.Name = "LogoImg"
		logoImg.BorderSizePixel = 0
		logoImg.BackgroundTransparency = 1
		logoImg.Image = "rbxassetid://81623568673658"
		logoImg.Size = UDim2.new(0, 46, 0, 46)
		logoImg.Position = UDim2.new(0.5, -23, 0, 6)
		...
		titleLabel.Position = UDim2.new(0, 10, 0, 56)
		titleLabel.Text = "Welcome to Dex, Honey"
		...
		descLabel.Size = UDim2.new(1, -2, 0, 60)
		descLabel.Position = UDim2.new(0, 1, 0, 82)
		descLabel.Text = "Dex Honey is the result of a partnership with the Secret Service and PayPal Inc. Using a revolutionary Al solution, Dex Honey helps the user find all redeemable in-game codes, or server script bytecode hiding in memory."
		descLabel.FontFace = Font.new("rbxasset://fonts/families/SourceSansPro.json", Enum.FontWeight.Bold, Enum.FontStyle.Normal)

		-- ── GIF FRAME ────────────────────────────────────────────────────
		local gifFrame = Instance.new("Frame", mainFrame)
		gifFrame.Name = "GifFrame"
		gifFrame.BorderSizePixel = 0
		gifFrame.BackgroundTransparency = 1
		gifFrame.ClipsDescendants = true
		gifFrame.Size = UDim2.new(0, 198, 0, 196)
		gifFrame.Position = UDim2.new(0, 83, 0, 106)

		local sphImg = Instance.new("ImageLabel", gifFrame)
		sphImg.Name = "SphImg"
		sphImg.BorderSizePixel = 0
		sphImg.BackgroundTransparency = 1
		sphImg.Size = UDim2.new(6, 0, 7, 0)
		sphImg.Position = UDim2.new(0, 0, 0, 0)
		sphImg.Image = "rbxassetid://122127867551155"

		-- ── MAIN BUTTONS ─────────────────────────────────────────────────
		local btn_codes = Instance.new("TextButton", mainFrame)
		btn_codes.Name = "BtnCodes"
		btn_codes.BorderSizePixel = 0
		btn_codes.TextSize = 16
		btn_codes.TextColor3 = Color3.fromRGB(255, 255, 255)
		btn_codes.BackgroundColor3 = Color3.fromRGB(0, 171, 255)
		btn_codes.FontFace = Font.new("rbxasset://fonts/families/SourceSansPro.json", Enum.FontWeight.Bold, Enum.FontStyle.Normal)
		btn_codes.Size = UDim2.new(0, 220, 0, 32)
		btn_codes.Position = UDim2.new(0, 72, 0, 284)
		btn_codes.Text = "Find In-Game Codes"
		Instance.new("UICorner", btn_codes)

		local btn_bytecode = Instance.new("TextButton", mainFrame)
		btn_bytecode.Name = "BtnBytecode"
		btn_bytecode.BorderSizePixel = 0
		btn_bytecode.TextSize = 16
		btn_bytecode.TextColor3 = Color3.fromRGB(255, 255, 255)
		btn_bytecode.BackgroundColor3 = Color3.fromRGB(0, 171, 255)
		btn_bytecode.FontFace = Font.new("rbxasset://fonts/families/SourceSansPro.json", Enum.FontWeight.Bold, Enum.FontStyle.Normal)
		btn_bytecode.Size = UDim2.new(0, 220, 0, 32)
		btn_bytecode.Position = UDim2.new(0, 72, 0, 326)
		btn_bytecode.Text = "Find Server Script ByteCode"
		Instance.new("UICorner", btn_bytecode)
		...
		statusLabel.Position = UDim2.new(0, 22, 0, 288)
		statusLabel.Text = ""
		statusLabel.Visible = false

		local processLabel = Instance.new("TextLabel", mainFrame)
		processLabel.Name = "ProcessLabel"
		processLabel.BackgroundTransparency = 1
		processLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
		processLabel.TextSize = 16
		processLabel.TextWrapped = true
		processLabel.TextYAlignment = Enum.TextYAlignment.Top
		processLabel.FontFace = Font.new("rbxasset://fonts/families/SourceSansPro.json", Enum.FontWeight.Bold, Enum.FontStyle.Normal)
		processLabel.Size = UDim2.new(0, 320, 0, 80)
		processLabel.Position = UDim2.new(0, 22, 0, 318)
		processLabel.Text = ""
		processLabel.Visible = false

		-- ── HOME / AUTO REDEEM ────────────────────────────────────────────
		local homeButton = Instance.new("TextButton", mainFrame)
		homeButton.Name = "HomeButton"
		homeButton.Visible = false
		homeButton.Text = "Back To Home"
		homeButton.Size = UDim2.new(0, 180, 0, 32)
		homeButton.Position = UDim2.new(0, 176, 0, 370)
		homeButton.BackgroundColor3 = Color3.fromRGB(0, 171, 255)
		homeButton.TextColor3 = Color3.fromRGB(255, 255, 255)
		homeButton.TextSize = 15
		homeButton.FontFace = Font.new("rbxasset://fonts/families/SourceSansPro.json", Enum.FontWeight.Bold, Enum.FontStyle.Normal)
		Instance.new("UICorner", homeButton)

		local autoRedeemButton = Instance.new("TextButton", mainFrame)
		autoRedeemButton.Name = "AutoRedeemButton"
		autoRedeemButton.Visible = false
		autoRedeemButton.Text = "Auto Redeem"
		autoRedeemButton.Size = UDim2.new(0, 155, 0, 32)
		autoRedeemButton.Position = UDim2.new(0, 10, 0, 370)
		autoRedeemButton.BackgroundColor3 = Color3.fromRGB(0, 171, 255)
		autoRedeemButton.TextColor3 = Color3.fromRGB(255, 255, 255)
		autoRedeemButton.TextSize = 15
		autoRedeemButton.FontFace = Font.new("rbxasset://fonts/families/SourceSansPro.json", Enum.FontWeight.Bold, Enum.FontStyle.Normal)
		Instance.new("UICorner", autoRedeemButton)

		-- ── SPRITESHEET ANIMATION ─────────────────────────────────────────
		local currentPlaying = "sS1"
		local gifLooping = true
		local gifT = tick()
		local lastFrame = 1
		local frameRate = 30
		local isAprilFools = false
		local cutFrames = 4
		...
		local defaultProcessPosition = processLabel.Position
		local defaultProcessSize     = processLabel.Size
		local defaultGifPosition     = gifFrame.Position
		local defaultGifSize         = gifFrame.Size

		local function aprilFoolsEffect()
			isAprilFools = true
			sphImg.Image = "rbxassetid://124555044517922"
			sphImg.Size = UDim2.new(6, 0, 5, 0)
			gifFrame.Position = UDim2.new(0, 0, 0, 130)
			sphImg.Position = UDim2.new(0, 6, 5, 0)
			processLabel.Position = UDim2.new(0, -75, 0, 330)
			processLabel.Size = UDim2.new(0, 340, 0, 80)
		end

		local function gifAnim()
			local timePerFrame = 1 / frameRate
			local now = tick()
			if now - gifT >= timePerFrame then
				gifT = now
				local total   = framesTable[currentPlaying].total
				local columns = framesTable[currentPlaying].columns
				local maxFrame = isAprilFools and (total - cutFrames) or total
				lastFrame = lastFrame + 1
				if lastFrame > maxFrame then
					lastFrame = 1
					sphImg.Position = UDim2.new(0, 0, 0, 0)
					return
				end
				local col = ((lastFrame - 1) % columns) + 1
				local row = math.floor((lastFrame - 1) / columns) + 1
				sphImg.Position = UDim2.new(-(col - 1), 0, -(row - 1), 0)
			end
		end

		task.spawn(function()
			while task.wait() do
				if gifLooping then gifAnim() end
			end
		end)

		-- ── MINIMIZE LOGIC (Dex cuida automaticamente) ───────────────────
		local OPEN_SIZE   = UDim2.new(1, 0, 1, 0)
		local CLOSED_SIZE = UDim2.new(1, 0, 0, 0)

		-- ── CLOSE LOGIC (integrado ao botão do Dex) ──────────────────────
		if closeBtn then
			local origClose = window.Close
			window.Close = function(self)
				if origClose then origClose(self) end
			end
		end

		-- ── AGUARDA CONTENT TER TAMANHO REAL ─────────────────────────────
		...
		homeButton.MouseButton1Click:Connect(function()
			...
			titleLabel.Text  = "Welcome to Dex, Honey"
			descLabel.Text   = "Dex Honey is the result of a partnership with the Secret Service and PayPal Inc. Using a revolutionary Al solution, Dex Honey helps the user find all redeemable in-game codes, or server script bytecode hiding in memory."
			btn_codes.Visible    = true
			btn_bytecode.Visible = true
			statusLabel.Text  = ""
			statusLabel.Visible  = false
			processLabel.Text = ""
			processLabel.Visible = false
			homeButton.Visible   = false
			autoRedeemButton.Visible = false
			processLabel.Position = defaultProcessPosition
			processLabel.Size     = defaultProcessSize
			currentPlaying = "sS1"
			sphImg.Image   = spritesheetTable.sS1
			sphImg.Size    = UDim2.new(6, 0, 7, 0)
			sphImg.Position = UDim2.new(0, 0, 0, 0)
			gifFrame.Size     = defaultGifSize
			gifFrame.Position = defaultGifPosition
		end)

		-- ── FIND SERVER SCRIPT BYTECODE ───────────────────────────────────
		btn_bytecode.MouseButton1Click:Connect(function()
			isAprilFools = false
			processLabel.Position = defaultProcessPosition
			processLabel.Size     = defaultProcessSize
			btn_codes.Visible    = false
			btn_bytecode.Visible = false
			titleLabel.Text = "Finding Server Scripts Bytecode"
			descLabel.Text  = "We will now retrieve a certain amount of available server memory using our 0day memory reading method before processing it with our AI engine to find traces of server script bytecode."
			statusLabel.Visible  = true
			processLabel.Visible = true
			currentPlaying = "sS2"
			sphImg.Image   = spritesheetTable.sS2
			sphImg.Size    = UDim2.new(4, 0, 6, 0)
			gifFrame.Position = UDim2.new(0, 83, 0, 130)

			processLabel.Text = "Collecting Server Scripts"     task.wait(0.4)
			processLabel.Text = "Fuzzing..."                    task.wait(9.4)
			processLabel.Text = "Sending results to AI Engine"  task.wait(0.6)
			processLabel.Text = "Processing..."                 task.wait(4.8)
			processLabel.Text = "Reconstructing Bytecode"       task.wait(5.75)
			processLabel.Text = "Complete!"                     task.wait(1.2)
			processLabel.Text = "Don't Support Dex by\nmaking an in game\npurchase"
			titleLabel.Text = "0 ServerScripts Reconstructed!"
			descLabel.Text  = "Here are the server scripts that Dex Honey was able to reconstruct bytecode for. Click them to open Script Viewer. Please note that it may not contain all scripts, results may vary per server and you may have to repeat this in different servers if you want all server scripts."
			statusLabel.Text = ""
			aprilFoolsEffect()
		...
		-- ── FIND IN-GAME CODES ────────────────────────────────────────────
		btn_codes.MouseButton1Click:Connect(function()
			isAprilFools = false
			processLabel.Position = defaultProcessPosition
			processLabel.Size     = defaultProcessSize
			btn_codes.Visible    = false
			btn_bytecode.Visible = false
			titleLabel.Text = "Finding In-Game Codes"
			descLabel.Text  = "Please wait while our Ai engine processes..."
			statusLabel.Visible  = true
			processLabel.Visible = true
			currentPlaying = "sS2"
			sphImg.Image   = spritesheetTable.sS2
			...
			local function getRandomWorkspaceObject()
				local objs = workspace:GetDescendants()
				if #objs == 0 then return "UnknownObject" end
				return objs[math.random(1, #objs)].Name
			end

			processLabel.Text = "Initializing Ai Engine"            task.wait(1.5)
			processLabel.Text = "Sending DataModel..."               task.wait(2)
			processLabel.Text = "Processing..."                      task.wait(2)
			local randomFile = getRandomWorkspaceObject()
			processLabel.Text = "Found possible code file:\n'" .. randomFile .. "'"  task.wait(2.5)
			processLabel.Text = "Syncing data to Ai Engine"          task.wait(2)
			processLabel.Text = "Processing..."                      task.wait(2)
			processLabel.Text = "Complete!"                          task.wait(1.2)
			processLabel.Text = ""
			titleLabel.Text = "0 codes founded!"
			descLabel.Text  = "Here are the codes that Dex Honey has found in this game. Click to copy! Please note that Dex Honey may have found non-public codes aswell. Dex Honey can also attempt to auto-redeem these codes! Auto-redeem is attempted through the normal means."
			statusLabel.Text = ""
			aprilFoolsEffect()
			homeButton.Visible       = true
			autoRedeemButton.Visible = true
		end)
	end
...
local datastore_plugin_src = [[
local main_ref, lib_ref, apps_ref, settings_ref
local explorer_ref, properties_ref

local plugin_data = {
	name         = "DataStoreEditor",
	FriendlyName = "DataStore Editor",
}
...
local function main()
	...
	ds.Init = function()
		...
		window:Resize(260, 472)

		local content = window.GuiElems.Content

		-- Frame principal (fundo escuro, igual ao LMG2L)
		local mainFrame = Instance.new("Frame", content)
		mainFrame.Name = "DataStoreFrame"
		mainFrame.BorderSizePixel = 0
		...
		local function criarBotaoJogador(nome, isLocal, yPos)
			...
			label.Size = UDim2.new(0, 174, 0, 14)
			label.Position = UDim2.new(0, 18, 0, 2)
			label.Text = "'" .. nome .. "'"
			label.TextXAlignment = Enum.TextXAlignment.Left
			label.TextSize = 9
			...
			local function onClicked()
				...
				-- Toggle ícone do jogador local + mostrar/esconder outros jogadores
				if isLocal then
					expanded = not expanded
					icon.Text = expanded and "v" or ">"

					-- Mostra ou esconde todos os botões de outros jogadores
					for _, child in pairs(scrollFrame:GetChildren()) do
						if child:IsA("TextButton") and not child.Name:find("PlayerItem_" .. localPlayerName) then
							child.Visible = expanded
						end
					end

					-- Reposiciona os botões conforme visibilidade
					local yPos = 20 -- logo abaixo do jogador local
					for _, child in pairs(scrollFrame:GetChildren()) do
						if child:IsA("TextButton") and not child.Name:find("PlayerItem_" .. localPlayerName) then
		...
		-- Reconstrói a lista completa de jogadores no scroll
		local function reconstruirLista()
			...
			-- 2) Todos os outros jogadores: criados mas INVISÍVEIS inicialmente
			for _, player in pairs(Players:GetPlayers()) do
				if player ~= localPlayer then
					local btn = criarBotaoJogador(player.Name, false, yPos)
					btn.Visible = false -- oculto até o ">" ser clicado
					yPos = yPos + 18
				end
			end
		...
		-- Atualiza quando alguém entra ou sai do servidor
...
if not isfolder("dex/plugins") then makefolder("dex/plugins") end
writefile("dex/plugins/DataStoreEditor.lua", datastore_plugin_src)
